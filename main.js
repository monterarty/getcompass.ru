...
const getSize = () => {
  var remSize = getComputedStyle(document.documentElement).getPropertyValue(
    "--remSize",
  );
  var factor = 0;
...
const getPage = () => {
  ...
  if (body.classList.contains("is--page-delete")) {
    return "delete-page";
  }
  if (window.location.href.indexOf("download") + 1) {
    return "download";
  }
  if (window.location.href.indexOf("mediakit") + 1) {
    return "mediakit";
  }
};
    ...
    switch (platform.os.family) {
      ...
      case "OS X":
        if (platform.version.split(".")[0] <= 13) {
          showOldBrowserMsg();
        } else if (platform.version.split(".")[0] <= 15) {
...
const downloadLinksOtherLink = document.createElement("div");
downloadLinksOtherLink.classList.add(
  "cta__list-item",
  "cta__list-other-button",
  "cursor-pointer",
  "w-inline-block",
);
const downloadLinksOtherIconWrap = document.createElement("div");
downloadLinksOtherIconWrap.classList.add(
  "cta__list-item-icon",
  "is--vector-full",
  "w-embed",
);
const downloadLinksOtherIcon = document.createElementNS(
  "http://www.w3.org/2000/svg",
  "svg",
);
downloadLinksOtherIcon.innerHTML =
...
var allDownloadDropdownsToggle = [];
...
const downloadLinks = [
  ...
  ...document.querySelectorAll(
    '.appstore, .playmarket, .huawei, .macintel, .macapple, .windows, .linuxdeb, .linuxtar, [data-platform="windows"]',
  ),
];
...
if (["iOS", "Android", "Huawei"].indexOf(os) + 1) {
  ...
  Array.prototype.forEach.call(ctaDropdowns, (ctaDropdown) => {
    ...
    if (
      !ctaDropdownList.classList.contains("is--desktop-platforms") &&
      !ctaDropdownList.classList.contains("is--mobile-platforms")
    ) {
      ...
      ctaDropdownToggle.addEventListener("click", () => {
        switch (os) {
          case "iOS":
            ...
            break;
          case "Android":
            buildLink = ctaDropdown.querySelector(".playmarket");
            buildUrl = ctaDropdown.querySelector(".playmarket").href;
            break;
          case "Huawei":
            buildLink = ctaDropdown.querySelector(".huawei");
            buildUrl = ctaDropdown.querySelector(".huawei").href;
            break;
        }
...
Array.prototype.forEach.call(downloadLinks, (downloadLink) => {
  var platform = Array.from(downloadLink.classList).find(
  ...
  input.addEventListener("keypress", function (e) {
    ...
    if (
      ...
      (input.value.length > 49 ||
        (e.key &&
          e.key.match(
            /[^а-яА-ЯЁёІіЇїҐґЄєa-zA-ZẞßÄäÜüÖöÀàÈèÉéÌìÍíÎîÒòÓóÙùÚúÂâÊêÔôÛûËëÏïŸÿÇçÑñœ’`'.-\s]/,
          )))
    )
  ...
  input.addEventListener("input", function (e) {
    if (e.inputType == "insertFromPaste") {
      ...
      input.value = input.value
        .replace(
          /[^а-яА-ЯЁёІіЇїҐґЄєa-zA-ZẞßÄäÜüÖöÀàÈèÉéÌìÍíÎîÒòÓóÙùÚúÂâÊêÔôÛûËëÏïŸÿÇçÑñœ’`'.-\s]/g,
          "",
        )
        .slice(0, 50);
    }
...
if (getPage() == "partner") {
  ...
  const calcResult = () => {
    ...
    сalcResultField.textContent = String(result.toFixed()).replace(
      /(\d)(?=(\d\d\d)+([^\d]|$))/g,
      "$1\u00A0",
    );
  };
