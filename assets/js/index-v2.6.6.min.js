(function(require$$0) {
  ...
  var DeviceDetectParser = new uaParserExports.UAParser(window.navigator.userAgent);
  class DeviceDetect {
    static isMobile() {
...
`;
  ...
  Array.prototype.forEach.call(downloadLinks, (downloadLink) => {
    ...
    downloadLink.addEventListener("click", (e) => {
      ...
      if (downloadLink.dataset.version === "cloud") {
        ...
        if (!mobileBodyClassNames.some(
          (className) => document.body.classList.contains(className)
        ) || mobileClassNames.some(
          (className) => downloadLink.classList.contains(className)
        ) || mobileClassNames.some(
          (className) => downloadLink.dataset.platform === className
        )) {
          e.preventDefault();
          const instructionLink = window.location.origin + instructionLinks[platform2];
          if (window.location.href.indexOf(instructionLinks[platform2]) + 1) {
            window.location.href = downloadLink.href;
          } else {
            window.location.href = instructionLinks[platform2] ? instructionLink : downloadLink.href;
          }
        }
      } else if (downloadLink.dataset.version === "onpremise") {
        getPage() === "on-premise" && sendEvent("305");
        switch (platform2) {
          case "appstore":
          case "huawei":
          case "playmarket":
      ...
      if (downloadLink.classList.contains(platform2) && getPage() !== "blog" && getPage() !== "download") {
        switch (platform2) {
          case "windows":
          case "windowsmsi":
          case "windows_old":
  ...
  if (mobileBodyClassNames.some(
    ...
    clipboard2.on("success", function(e) {
      showCopyNote();
      if (["download", "blog", "post"].indexOf(getPage()) === -1 && e.trigger.dataset.version === "cloud") {
        sendEvent("25");
      }
  ...
  Array.prototype.forEach.call(CTADropdowns, (dropdown) => {
    ...
    dropdownToggle.addEventListener("click", () => {
      ...
      if (isWindowsList) {
        const isMobileUser = mobileBodyClassNames.some(
          (className) => document.body.classList.contains(className)
        );
