...
`;
  ...
  if (window.location.hash) {
    ...
    if (targetEl) {
      ...
      window.scrollTo({
        top: targetEl.offsetTop - navbarHeight - 320 * getSize(),
        // Учитываем отступ
        behavior: "smooth"
      });
    }
  ...
  window.addEventListener("scroll", () => {
    ...
    if (["partner", "on-premise"].indexOf(getPage()) + 1 && window.scrollY > getSize() * 47.13 || getPage() === "home" && window.scrollY > getSize() * 51.66 || getPage() === "download" && window.scrollY > getSize() * 25 || getPage() === "security" && window.scrollY > getSize() * 42.6) {
      if ($(window).width() < 768) {
        if (getPage() === "partner") {
          setTimeout(() => {
